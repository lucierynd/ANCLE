...
def generate_launch_description():
    ...
    nav2 = IncludeLaunchDescription(
                ...
                launch_arguments={'params_file': os.path.join(
                    get_package_share_directory('ancle_pkg'), 'params', 'nav2_config.yaml')}.items()
    )

    # Wait that everything is settled before launching slam and nav2
    ...
    rviz = Node(
        ...
        arguments=['-d', os.path.join(
            get_package_share_directory('ancle_pkg'), 'rviz', 'rviz_nav2_multi_lidar_inertial_slam_config.rviz')],
        output='screen',
