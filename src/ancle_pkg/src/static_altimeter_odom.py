...
class AltimeterToOdom(Node):
    def __init__(self):
        ...
        # Parameters
        self.declare_parameter('alt_odom_topic', '/altimeter_odom')
        ...
        alt_odom_topic = self.get_parameter('alt_odom_topic').value
        ...
        # Publisher
        self.publisher = self.create_publisher(Odometry, alt_odom_topic, 10)
        timer_period = 0.05  # seconds (20 Hz)
        self.timer = self.create_timer(timer_period, self.altimeter_callback)
        self.get_logger().info(f'Publishing static altitude data in {alt_odom_topic}')

    def altimeter_callback(self):
        odom = Odometry()
